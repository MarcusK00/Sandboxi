...
">[Right Click] eraser</div>
    ...
    ">
      ...
      <div id="material-fuse" style="cursor:pointer; color: rgb(65, 66, 87)">Fuse</div>
      <div id="material-ice" style="cursor:pointer; color: rgb(185,232,234)">Ice</div>
      <div id="material-mud" style="cursor:pointer; color: rgb(81,60,36)">Mud</div>
      <div id="material-smoke" style="cursor:pointer; color: rgb(71,71,71)">Smoke</div>
      <div id="material-steam" style="cursor:pointer; color: rgb(210,219,220)">Steam</div>
      <div id="material-wood" style="cursor:pointer; color: rgb(150, 110, 51)">Wood</div>
    </div>
  `;
  ...
  document.getElementById("material-fuse")!.addEventListener("click", () => {
    onMaterialSelect("fuse"); select("fuse");
  });

  document.getElementById("material-ice")!.addEventListener("click", () => {
    onMaterialSelect("ice"); select("ice");
  });

  document.getElementById("material-mud")!.addEventListener("click", () => {
    onMaterialSelect("mud"); select("mud");
  });

  document.getElementById("material-smoke")!.addEventListener("click", () => {
    onMaterialSelect("smoke"); select("smoke");
  });

  document.getElementById("material-steam")!.addEventListener("click", () => {
    onMaterialSelect("steam"); select("steam");
  });

  document.getElementById("material-wood")!.addEventListener("click", () => {
    onMaterialSelect("wood"); select("wood");
  });
}
